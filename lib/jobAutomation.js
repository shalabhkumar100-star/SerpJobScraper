...
export function buildJobDigest(jobs, { runKey }) {
  ...
  const urgent = sorted.filter((job) => {
    const deadline = daysUntil(job.deadlineDate);
    return scoreJob(job) >= Number(process.env.HIGH_PRIORITY_JOB_SCORE || 5) || (deadline !== null && deadline >= 0 && deadline <= Number(process.env.URGENT_DEADLINE_DAYS || 5));
  });
  ...
  return {
    ...
    urgentJobs: urgent.length,
    sourceCounts,
    topJobs: sorted.slice(0, Number(process.env.NOTIFICATION_TOP_JOBS || 10)),
    urgent: urgent.slice(0, Number(process.env.NOTIFICATION_URGENT_JOBS || 5)),
  };
}

function digestText(digest) {
  const lines = [
    `Weekly job search finished: ${digest.runKey}`,
    `${digest.totalJobs} roles in Notion. ${digest.urgentJobs} high-priority or deadline-sensitive.`,
    `Sources: ${Object.entries(digest.sourceCounts).map(([source, count]) => `${source}: ${count}`).join(", ") || "none"}`,
    "",
    "Top jobs:",
    ...digest.topJobs.map((job, index) => `${index + 1}. ${job.role} at ${job.company}${job.deadlineDate ? ` | deadline ${job.deadlineDate}` : ""}${job.url ? ` | ${job.url}` : ""}`),
  ];
  return lines.join("\n").slice(0, 3500);
}
...
export async function sendJobNotifications(digest) {
  ...
  const subject = `Weekly job search: ${digest.urgentJobs} priority roles, ${digest.totalJobs} total`;
...
export async function sendContentReminder({ runKey }) {
  const text = `Weekly content generation reminder: job-market signals are ready for ${runKey}. Generate/refine LinkedIn drafts in the content generator and save them to Notion for review.`;
  const results = await Promise.allSettled([
    notifyTelegram(text),
